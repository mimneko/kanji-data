...
文化庁発表の常用漢字表[^1][^2]をもとに構成した、常用漢字 2136 字種とその異体字の JSON データ。
- 漢字
- 音読み・訓読み
- 用例
...
日本産業企画（JIS）の JIS X 0213 漢字一覧の漢字 10050 字の CSV データ
- 漢字
- JIS面区点コード
- ユニコード
- JIS第何水準か
...
子の名に使える漢字[^4] 2999(2769字種2999字体) 字を整理した CSV データ
- 戸籍・命名において使用可能な漢字
...
漢字の部首を整理した CSV データ。ここでは漢検漢字辞典[^3]の示した部首をまとめた。
- 部首
- 名称
- 画数
- ユニコード
...
> [!NOTE]
> 康熙字典の部首も 214 種類であるが、漢検漢字辞典では「⼡(U+2F21)」「⼢(U+2F22)」をまとめて「⼡(U+2F21)」とし、また「⺍(U+2E8D)」を追加して 214 種類となっているので、内訳は異なる。
